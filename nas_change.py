...
class Encoder(pynn.PyroModule):
    def __init__(self, input_dim, hidden_dim, latent_dim, debug=False):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.debug:
            print(f"Encoder hidden layer shape: {h.shape}")
        mean = self.fc2_mean(h)
        logvar = self.fc2_logvar(h)
        if self.debug:
            print(f"Encoder output mean shape: {mean.shape}, logvar shape: {logvar.shape}")
        return mean, logvar

# Define the Decoder part of the BVAE
class Decoder(pynn.PyroModule):
    def __init__(self, latent_dim, hidden_dim, output_dim, debug=False):
        super().__init__()
    ...
    def forward(self, z):
        ...
        if self.debug:
            print(f"Decoder hidden layer shape: {h.shape}")
...
class BVAE(pynn.PyroModule):
    def __init__(self, input_dim, hidden_dim, latent_dim, debug=False):
        super().__init__()
        self.encoder = Encoder(input_dim, hidden_dim, latent_dim, debug)
        self.decoder = Decoder(latent_dim, hidden_dim, input_dim, debug)
    ...
    def model(self, x):
        ...
        with pyro.plate("data", x.shape[0]):
            ...
            if self.debug:
                print(f"Sampled latent z shape: {z.shape}")
    ...
    def guide(self, x):
        ...
        with pyro.plate("data", x.shape[0]):
            ...
            if self.debug:
                print(f"Guide z_loc shape: {z_loc.shape}, z_scale shape: {z_scale.shape}")
    ...
    def forward(self, x, *args, **kwargs):
        ...
        if self.debug:
            print(f"Forward sampled latent z shape: {z.shape}")
        return self.decoder(z)

# Define the model function for BVAE
def model_bvae(trial, input_dim, output_dim, debug=False):
    # Sample hyperparameters
    hidden_dim = trial.suggest_int('hidden_dim', 50, 200)
    latent_dim = trial.suggest_int('vae_latent_dim', 10, 50)

    # Instantiate BVAE model
    bvae_model = BVAE(input_dim, hidden_dim, latent_dim, debug)
    ...
    def model(features, targets=None):
        ...
        for i, layer in enumerate(mc_dropout_model.layers):
            if isinstance(layer, pynn.PyroModule[nn.Linear]):
                ...
                layer.weight = pyro.nn.PyroParam(pyro.sample(f"w_{i}", weight_prior))
                layer.bias = pyro.nn.PyroParam(pyro.sample(f"b_{i}", bias_prior))
                print(f"Sampled w_{i} shape: {layer.weight.shape}")  # Debugging: print shape of sampled weights
                print(f"Sampled b_{i} shape: {layer.bias.shape}")   # Debugging: print shape of sampled biases
        ...
        print(f"Sampled w_output shape: {mc_dropout_model.output_layer.weight.shape}")  # Debugging: print shape of sampled weights
        print(f"Sampled b_output shape: {mc_dropout_model.output_layer.bias.shape}")    # Debugging: print shape of sampled biases

        # Define the likelihood
        with pyro.plate("data", features.shape[0]):
            outputs = mc_dropout_model(features)
            print(f"Model outputs shape: {outputs.shape}")  # Debugging: print shape of model outputs
            pyro.sample("obs", dist.Normal(outputs, 1), obs=targets)

    return model

# Example usage of model_mc_dropout in an objective function
# (actual implementation of objective function should follow)
# input_dim and output_dim need to be defined based on dataset
input_dim = 214  # excluding target column
output_dim = 1  # assuming single target variable                                                                                                 
...
def model_vbnn(trial, input_dim, output_dim):
    ...
    def model(features, targets=None):
        ...
        with pyro.plate("data", features.shape[0]):
            ...
            print(f"Sampled w_output shape: {vbnn_model.output_layer.weight.shape}")  # Debugging: print shape of sampled weights
            print(f"Sampled b_output shape: {vbnn_model.output_layer.bias.shape}")    # Debugging: print shape of sampled biases

            # Define the likelihood
            print("Input data shape:", features.shape)  # Debugging: print input data shape
